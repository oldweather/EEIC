# Make a synthesis figure for the main fleet in the South Pacific
library(grid)
library(chron)
library(lattice)
library(maps)
...
# Read in the data
sts.1<-read.table('../Alnwick_Castle_1804-5/ovn.out',header=F)
sts.1$Date<-chron(dates=as.character(sts.1$V1),times=as.character(sts.1$V2),
                           format=c(dates = "y/m/d", times = "h:m:s"))
sts.2<-read.table('../Arniston_1804-5/ovn.out',header=F)
sts.2$Date<-chron(dates=as.character(sts.2$V1),times=as.character(sts.2$V2),
                           format=c(dates = "y/m/d", times = "h:m:s"))
sts.3<-read.table('../Ceres_4_1804-5/ovn.out',header=F)
sts.3$Date<-chron(dates=as.character(sts.3$V1),times=as.character(sts.3$V2),
                           format=c(dates = "y/m/d", times = "h:m:s"))
sts.4<-read.table('../Cuffnells_1804-5/ovn.out',header=F)
sts.4$Date<-chron(dates=as.character(sts.4$V1),times=as.character(sts.4$V2),
                           format=c(dates = "y/m/d", times = "h:m:s"))
sts.5<-read.table('../Royal_Charlotte_5_1804-5/ovn.out',header=F)
sts.5$Date<-chron(dates=as.character(sts.5$V1),times=as.character(sts.5$V2),
                           format=c(dates = "y/m/d", times = "h:m:s"))
tics = dates(c('1804/10/30','1804/11/10','1804/11/20','1804/11/30'),
      format="y/m/d")
ticl = c('1804/10/30','1804/11/10','1804/11/20','1804/11/30')
      
postscript(file="sp_combi.ps",paper="a4",family="Helvetica",pointsize=12)
...
#}
downViewport("vp_map")
    pushViewport(plotViewport(margins=c(2,5,2,5)))
    pushViewport(dataViewport(c(140,180),c(-50,0)))
    grid.xaxis(at=c(140,150,160,170,180),main=T)
    grid.xaxis(at=c(140,150,160,170,180),main=F)
    grid.text('Longitude',y=unit(-3,"lines"))
    grid.yaxis(at=c(-50,-40,-30,-20,-10,0),main=T)
    grid.yaxis(at=c(-50,-40,-30,-20,-10,0),main=F)
    grid.text('Latitude',x=unit(-3.5,"lines"), rot=90)
    popViewport()
    pushViewport(dataViewport(c(140,180),c(-50,0),clip='on'))
...
pushViewport(dataViewport(as.numeric(c(tics[1],tics[4])),c(5,30)))
...
popViewport()
pushViewport(dataViewport(as.numeric(c(tics[1],tics[4])),c(5,30),clip='on'))
...
downViewport("vp_sst")
pushViewport(plotViewport(margins=c(2,2,2,2)))
pushViewport(dataViewport(as.numeric(c(tics[1],tics[4])),c(0,30)))
grid.xaxis(at=as.numeric(tics),label=ticl,main=T)
grid.text('Date',y=unit(-3,"lines"))
grid.yaxis(main=T)
grid.text('Wind Speed (m/s)',x=unit(-3.5,"lines"), rot=90)
popViewport()
pushViewport(dataViewport(as.numeric(c(tics[1],tics[4])),c(0,30),clip='on'))
...
downViewport("vp_slp")
pushViewport(plotViewport(margins=c(2,2,2,2)))
pushViewport(dataViewport(as.numeric(c(tics[1],tics[4])),c(990,1030)))
grid.xaxis(at=as.numeric(tics),label=ticl,main=T)
grid.text('Date',y=unit(-3,"lines"))
grid.yaxis(main=T)
grid.text('Pressure (hPa)',x=unit(-3.5,"lines"), rot=90)
popViewport()
pushViewport(dataViewport(as.numeric(c(tics[1],tics[4])),c(990,1030),clip='on'))
