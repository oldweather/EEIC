# Make a synthesis figure for the main fleet On the voyage out
library(grid)
library(chron)
library(lattice)
library(maps)
...
for(d in unique(c(sts.1$Date,sts.2$Date,sts.3$Date,sts.4$Date,sts.5$Date))) {
   m<-mean(c(sts.1$V14[sts.1$Date==d],sts.2$V14[sts.2$Date==d],
               sts.3$V14[sts.3$Date==d],sts.4$V14[sts.4$Date==d],
               sts.5$V14[sts.5$Date==d]),na.rm=T)
   if(length(sts.1$V14[sts.1$Date==d])>0) {
      sts.1$V14[sts.1$Date==d]<-sts.1$V14[sts.1$Date==d]-m
   }  
   if(length(sts.2$V14[sts.2$Date==d])>0) {
      sts.2$V14[sts.2$Date==d]<-sts.2$V14[sts.2$Date==d]-m
   }  
   if(length(sts.3$V14[sts.3$Date==d])>0) {
      sts.3$V14[sts.3$Date==d]<-sts.3$V14[sts.3$Date==d]-m
   }  
   if(length(sts.4$V14[sts.4$Date==d])>0) {
      sts.4$V14[sts.4$Date==d]<-sts.4$V14[sts.4$Date==d]-m
   }  
   if(length(sts.5$V14[sts.5$Date==d])>0) {
      sts.5$V14[sts.5$Date==d]<-sts.5$V14[sts.5$Date==d]-m
   } 
}
...
pdf(file="longitude_v_time.pdf",paper="special",
        width=6,height=4,family="Helvetica",pointsize=12)
...
grid.yaxis(main=T)
grid.text('Longitude anomaly (degrees)',x=unit(-3.5,"lines"), rot=90)
popViewport()
pushViewport(dataViewport(as.numeric(sts.1$Date),c(-5,5),clip='on'))
    ...
    grid.points(y=unit(sts.1$V14,"native"),x=unit(sts.1$Date,'native'),pch=20,
               size=unit(2,"native"),gp=gp_red)
    grid.points(y=unit(sts.2$V14,"native"),x=unit(sts.2$Date,'native'),pch=20,
               size=unit(2,"native"),gp=gp_blue)
    grid.points(y=unit(sts.3$V14,"native"),x=unit(sts.3$Date,'native'),pch=20,
               size=unit(2,"native"),gp=gp_black)
    grid.points(y=unit(sts.4$V14,"native"),x=unit(sts.4$Date,'native'),pch=20,
               size=unit(2,"native"),gp=gp_green)
    grid.points(y=unit(sts.5$V14,"native"),x=unit(sts.5$Date,'native'),pch=20,
