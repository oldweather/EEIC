# Make a synthesis figure for the Walpole
library(grid)
library(chron)
library(lattice)
library(maps)
...
sts2$Date<-chron(dates=as.character(sts2$V1),times=as.character(sts2$V2),
                           format=c(dates = "y/m/d", times = "h:m:s"))
tics = dates(c('1794/06/01','1794/09/01','1794/12/01','1795/03/01','1795/06/01','1795/09/01','1795/12/01'),
      format="y/m/d")
ticl = c('1794/06','1794/09','1794/12','1795/03','1795/06','1795/09','1795/12')
      
postscript(file="Walpole.ps",paper="a4",family="Helvetica",pointsize=12)
...
downViewport("vp_at")
pushViewport(plotViewport(margins=c(2,2,2,2)))
pushViewport(dataViewport(as.numeric(c(tics[1],tics[6])),c(5,30)))
...
downViewport("vp_sst")
pushViewport(plotViewport(margins=c(2,2,2,2)))
pushViewport(dataViewport(as.numeric(c(tics[1],tics[6])),c(0,30)))
grid.xaxis(at=as.numeric(tics),label=ticl,main=T)
grid.text('Date',y=unit(-3,"lines"))
grid.yaxis(main=T)
...
downViewport("vp_slp")
pushViewport(plotViewport(margins=c(2,2,2,2)))
pushViewport(dataViewport(as.numeric(c(tics[1],tics[6])),c(990,1030)))
grid.xaxis(at=as.numeric(tics),label=ticl,main=T)
grid.text('Date',y=unit(-3,"lines"))
grid.yaxis(main=T)
