...
for(i in seq(2,length(hts))) { 
   ...
   eval(parse(text=sety))
   year = as.numeric(sub ('/.*','',hts$Date))
   month = as.numeric(substr(as.character(hts$Date),6,7))
   x=append(x,year+(month-0.5)/12)
   y=append(y,yd)
}
hm<-rep(0.0,(1836-1790)*12)
hy<-seq(1790+0.5/12,1835+11.5/12,1/12)
...
nm<-rep(0.0,(1836-1790)*12)
ny<-seq(1790+0.5/12,1835+11.5/12,1/12)
...
postscript(file="compare_at_anom_HadCM3_monthly.ps",paper="a4",family='Helvetica',pointsize=16)
...
popViewport()
pushViewport(dataViewport(c(1790,1835),c(-1.5,1.0),clip='on'))
...
grid.lines(x=unit(c(1815,1815),'native'),
           y=unit(c(-2,2),'native'),
           gp=mgp)
grid.points(x=unit(hy,'native'),
            y=unit(hm-0.1,'native'),
           size=unit(0.01,'npc'),
           pch=20,
           gp=hgp)
grid.points(x=unit(ny,'native'),
