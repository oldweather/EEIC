...
# Find dates and positions for all SLP observations
my %Obs;
foreach (@ARGV) {
...
my $Maxlen=0;
open( DIN,
    "/home/h03/hadpb/tasks/soap/simons_runs/variables/slp/all/monthly/anomaly.pp"
    ...
    for ( my $Month = 1 ; $Month <= 12 ; $Month++ ) {
        ...
        for ( my $i = 0 ; $i < $Maxlen ; $i++ ) {
            if ( defined( $Pseudo_obs{$Dstring}[$i] ) ) {
                printf " %7.1f", $Pseudo_obs{$Dstring}[$i];
            }
            else {
                print "      NA";
            }
