# Make a synthesis figure for the Thomas Grenville
library(grid)
library(chron)
library(lattice)
library(maps)
...
# Read in the data
sts<-read.table('tg.ovn.out',header=F)
sts$Date<-chron(dates=as.character(sts$V1),times=as.character(sts$V2),
                           format=c(dates = "y/m/d", times = "h:m:s"))
sts2<-read.table('tg.ovn.back',header=F)
sts2$Date<-chron(dates=as.character(sts2$V1),times=as.character(sts2$V2),
                           format=c(dates = "y/m/d", times = "h:m:s"))
tics = dates(c('1827/06/01','1827/09/01','1827/12/01','1828/03/01','1828/06/01','1828/09/01'),
      format="y/m/d")
ticl = c('1827/06','1827/09','1827/12','1828/03','1828/06','1828/09')
      
postscript(file="Thomas_Grenville.ps",paper="a4",family="Helvetica",pointsize=12)
                    
# Four seperate plots - route, AT, SLP, SST - each in its own viewport
...
grid.text('Sea Temperature (C)',x=unit(-3.5,"lines"), rot=90)
   ...
   grid.points(x=unit(sts$Date,"native"),y=unit(sts$V5,'native'),pch=20,
               size=unit(10,"native"),gp=gp_red)
   grid.lines(x=unit(sts2$Date,"native"),y=unit(sts2$V6,'native'),gp=gp_black)
   grid.points(x=unit(sts2$Date,"native"),y=unit(sts2$V5,'native'),pch=20,
               size=unit(10,"native"),gp=gp_blue)
popViewport() 
popViewport() 
upViewport()
