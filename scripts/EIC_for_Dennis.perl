...
for ( my $i = 1 ; $i < scalar(@Records) - 1 ; $i++ ) {
    ...
    if (   defined( $Records[$i]->{LON} )
        && defined( $Records[ $i - 1 ]->{LON} )
        && abs( $Records[$i]->{LON} - $Records[ $i - 1 ]->{LON} ) / $Dd > 3
...
# Output data from the records of interest to Dennis
print "Ship name,Year,Month,Day,Latitude,Longitude,".
      "Keyed wind direction,Magnetic wind direction,".
      "True wind direction,Keyed wind descriptors,".
      "Wind descriptors as text,Beaufort wind force,".
      "Wind speed (m/s),Keyed air temperature,".
      "Converted air temperature (C),Keyed pressure,",
      "Converted pressure (hPa)\n";

foreach my $Ob (@Records) {

    # Keep only the observations of interest to Dennis
    ...
    printf "%-30s,",
      TruncateNames::formatted_from_long(
        TruncateNames::long_from_short( $Ob->{ID} ) );
    printf "%04d,%02d,%02d,", $Ob->{YR}, $Ob->{MO}, $Ob->{DY};
    printf "%5.1f,%6.1f,", $Ob->{LAT}, $Ob->{LON};
    printf "%10s,", $Ob->{Fields}->[20];
    if ( $Ob->{Fields}->[20] =~ /\S/ ) {
        $Ob->{D2} = decode_direction( $Ob->{Fields}->[20] );
    }
    if ( defined( $Ob->{D2} ) && $Ob->{D2} <= 360 ) {
        printf "%3d,", $Ob->{D2};
    }
    else { print "   ,"; }
    if ( defined( $Ob->{D} ) && $Ob->{D} <= 360 ) {
        printf "%3d,", $Ob->{D};
    }
    else { print "   ,"; }
    printf "%10s,", $Ob->{Fields}->[22];
    my ( $Words, $Force );
    if ( $Ob->{Fields}->[22] =~ /\S/ ) {
        $Ob->{Fields}->[22] =~ /(...)(...)/;
        $Words = join( " ", WindsEIC::ncdc_to_words( $1, $2 ) );
        $Force = WindsEIC::ncdc_to_beaufort( $1, $2 );
    }
    if ( defined($Words) ) {
        printf "%20s,", $Words;
    }
    ...
    printf "%5s,", $Ob->{Fields}->[18].$Ob->{Fields}->[19];
    if ( defined( $Ob->{AT} ) ) {
        printf "%4.1f,", $Ob->{AT};
    }
    else { print "    ,"; }
    printf "%5s,", $Ob->{Fields}->[16];
    if ( defined( $Ob->{SLP} ) ) {
        printf "%5.1f", $Ob->{SLP};
    }
    else { print "    "; }
