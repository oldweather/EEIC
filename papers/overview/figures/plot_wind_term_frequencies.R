...
max.length=70 # Don't clutter the plot with rare values
...
pdf(file="wind_term_frequencies.pdf",paper='special',pointsize=8,
         width=6,height=8)
...
for(i in seq(1,min(length(sts$V1),max.length))) {
    if(is.na(sts$V4[i])) {
        grid.points(x=unit(sts$V3[i],"native"),
                    y=unit(y[i],"native"),
                    pch=20,size=unit(0.3,"native"),gp=ggp)
    }
    else {
        grid.points(x=unit(sts$V3[i],"native"),
                    y=unit(y[i],"native"),
                    pch=20,size=unit(0.3,"native"),gp=bgp)
    }
}    
