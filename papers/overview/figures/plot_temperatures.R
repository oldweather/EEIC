# R script to plot map and time-series of temperatures
library(grid)
library(lattice)
library(PP.map)

pdf(file='temperatures.pdf',width=6,height=8,pointsize=12)
...
pushViewport(viewport(width=1.0,height=0.7,x=0.0,y=0.35,
   ...
   m<-pp.map(f[[1]],lon_range=c(-60,180),
          draw=F,
          levels=c(-5,-4,-3,-2,-1,-.5,.5,1,2,3,4,5))
   print(m,newpage=F)
upViewport()

# Mean temperature in each year
sts<-read.table('../../../analysis/anomaly_ts/at_anoms.out',header=T)
x=rep(0,0)
y=rep(0,0)
...
pushViewport(dataViewport(c(1790,1835),c(-1,1)))
grid.xaxis(main=T)
grid.text('Year',y=unit(-3,"lines"))
grid.yaxis(main=T)
grid.text('AT anomaly (C)',x=unit(-3,"lines"), rot=90)
popViewport()
pushViewport(dataViewport(c(1790,1835),c(-1,1),clip='on'))
