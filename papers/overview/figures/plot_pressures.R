# R script to plot map and time-series of pressures
library(grid)
library(lattice)
library(PP.map)

pdf(file='pressures.pdf',width=6,height=8,pointsize=12)
...
pushViewport(viewport(width=1.0,height=0.7,x=0.0,y=0.35,
   ...
   m<-pp.map(f[[3]],lon_range=c(-60,180),
          draw=F,
          levels=c(-20,-10,-8,-6,-4,-2,-1,1,2,4,6,8,10,20))
   print(m,newpage=F)
upViewport()

# Mean pressure in each year
sts<-read.table('../../../analysis/anomaly_ts/pre_anoms.out',header=T)
x=rep(0,0)
y=rep(0,0)
...
for(i in seq(1,length(yy))) {
 ym[i]<-mean(y[x==yy[i]],trim=0.0,na.rm=T)
}
...
pushViewport(dataViewport(c(1790,1835),c(-15,5)))
grid.xaxis(main=T)
grid.text('Year',y=unit(-3,"lines"))
grid.yaxis(main=T)
grid.text('Pressure anomaly (hPa)',x=unit(-3,"lines"), rot=90)
popViewport()
pushViewport(dataViewport(c(1790,1835),c(-15,5),clip='on'))
